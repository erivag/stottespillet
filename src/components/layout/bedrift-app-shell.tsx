...
const NAV = [
  { href: "/bedrift/dashboard", label: "Hjem", emoji: "🏠" },
  { href: "/bedrift/innboks", label: "Innboks", emoji: "📬" },
  { href: "/bedrift/budsjett", label: "Budsjett", emoji: "💰" },
  { href: "/bedrift/rapporter", label: "Rapporter", emoji: "📊" },
  { href: "/bedrift/innstillinger", label: "Innstillinger", emoji: "⚙️" },
] as const;
...
export function BedriftAppShell({ children }: { children: ReactNode }) {
  return (
    <div className="min-h-dvh bg-[var(--brand-cream)] pb-[4.5rem] lg:pb-0">
      <aside className="fixed left-0 top-0 z-40 hidden h-full w-56 flex-col border-r border-[var(--brand-pine)]/10 bg-white shadow-sm lg:flex">
        <div className="border-b border-[var(--brand-pine)]/10 px-5 py-6">
          ...
          <p className="mt-1 text-xs text-neutral-500">Bedrift</p>
        </div>
