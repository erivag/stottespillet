...
export async function sendShopBookingEmails(
  ...
  const discountLine =
    payload.discountPercent > 0
      ? `<p><strong>Volumrabatt:</strong> ${payload.discountPercent}%</p>
         <p><strong>Sum før rabatt:</strong> ${escHtml(payload.subtotalKrFormatted)}</p>`
      : "";
  ...
  <p><strong>Pris per dusin:</strong> ${escHtml(payload.unitPriceKrFormatted)}</p>
  ${discountLine}
  <p><strong>Totalt (etter rabatt):</strong> ${escHtml(payload.totalKrFormatted)}</p>
